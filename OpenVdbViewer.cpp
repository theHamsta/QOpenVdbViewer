...
void OpenVdbViewer::draw()
{
	...
	glBegin( GL_QUADS );
	glTexCoord2f( 0.0, 0.0 ); glVertex3f( x_pos, y_pos, 0.0f );
	glTexCoord2f( 0.0, 1.0 ); glVertex3f( x_pos, y_pos + geometry().height(), 0.0f );
	glTexCoord2f( 1.0, 1.0 ); glVertex3f( x_pos + geometry().width(), y_pos + geometry().height(), 0.0f );
	glTexCoord2f( 1.0, 0.0 ); glVertex3f( x_pos + geometry().width(), y_pos, 0.0f );
	glEnd();
