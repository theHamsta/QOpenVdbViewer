...
QOpenVdbViewer::QOpenVdbViewer(QWidget* parent) : QGLViewer(parent)
{
...
void QOpenVdbViewer::draw()
{
    ...
    // ) ); m_gvdb->getScene()->LinearTransferFunc ( 0.50f, 0.75f, Vector4DF( 1, 0, 0, 0.02f ), Vector4DF( 1, .5f, 0,
    // 0.01f ) ); m_gvdb->getScene()->LinearTransferFunc ( 0.75f, 1.00f, Vector4DF( 1, .5f, 0, 0.01f ), Vector4DF( 0, 0,
    // 0, 0.005f ) );
...
static void checkGL(const char* msg)
{
    ...
    if (errCode != GL_NO_ERROR) { printf("%s, ERROR: 0x%x\n", msg, errCode); }
}
...
void QOpenVdbViewer::init()
{
    ...
    // 	char scnpath[1024];
    //
    // 	if ( !m_gvdb->getScene()->FindFile ( "/local/xu29mapu/projects/gvdb-voxels/source/shared_assets/" "bunny.vdb",
    // scnpath ) ) { 		gprintf ( "Cannot find vdb file.\n" ); 		gerror();
    // 	}
...
{
    if (m_grid) { m_gvdb->LoadVDB(m_grid); }
}
